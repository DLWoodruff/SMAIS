...
def pysp_instance_creation_callback(
    ...
    # model.obj = pyo.Objective(rule=total_cost_rule, sense=sense)
...
def g_calc(d, base_model, cfg, xhat):
    ...
    for i in range(len(d)//3):
        base_model.Yield[f"WHEAT{i}"] = d[i]
        base_model.Yield[f"CORN{i}"] = d[3*i+1]
        base_model.Yield[f"SUGAR_BEETS{i}"] = d[3*i+2]
    nlens = {"ROOT":len(d)}
    return solve_fixed_up_to_stage(base_model, 1, xhat, nlens, cfg.solver_name, cfg.solver_options)

def p_calc(d, cfg):
    d = np.array(d)
    wheat_bases = np.array([2, 2.5, 3])
    corn_bases = np.array([2.4, 3, 3.6])
    beets_bases = np.array([16, 20, 24])
    
    WHEAT_BD = _get_b(2.5, cfg.yield_cv)
    CORN_BD = _get_b(3, cfg.yield_cv)
    BEETS_BD = _get_b(20, cfg.yield_cv)

    if np.isscalar(d[0]):
        prob = 1
        for i in range(len(d)//3):
            wheat_prob = np.sum((d[i*3] >= wheat_bases) & (d[i*3] <= wheat_bases + WHEAT_BD)) / (3 * WHEAT_BD)
            corn_prob = np.sum((d[i*3+1] >= corn_bases) & (d[i*3+1] <= corn_bases + CORN_BD)) / (3 * CORN_BD)
            beets_prob = np.sum((d[i*3+2] >= beets_bases) & (d[i*3+2] <= beets_bases + BEETS_BD)) / (3 * BEETS_BD)
            
            # Multiply the probabilities for the current set of wheat, corn, and beets
            prob *= wheat_prob * corn_prob * beets_prob
        return prob
    else:       
        # Calculate probabilities for each crop type
        wheat_probs = np.sum((d[:, ::3] >= wheat_bases) & 
                            (d[:, ::3] <= wheat_bases + WHEAT_BD), axis=1) / (3 * WHEAT_BD)
        corn_probs = np.sum((d[:, 1::3] >= corn_bases) & 
                            (d[:, 1::3] <= corn_bases + CORN_BD), axis=1) / (3 * CORN_BD)
        beets_probs = np.sum((d[:, 2::3] >= beets_bases) & 
                            (d[:, 2::3] <= beets_bases + BEETS_BD), axis=1) / (3 * BEETS_BD)
        
        # Calculate overall probability for each set in d
        total_prob = wheat_probs * corn_probs * beets_probs
...
def find_limits(cfg):
    d_limits = []
    for i in range(cfg.crops_multiplier):
        d_limits.append([2, 3+_get_b(2.5, cfg.yield_cv)])
        d_limits.append([2.4, 3.6+_get_b(3, cfg.yield_cv)])
        d_limits.append([16, 24+_get_b(20, cfg.yield_cv)])
    return d_limits
