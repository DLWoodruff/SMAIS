...
def g_calc(d, base_model, cfg, xhat):
    ...
    dd = {i+1: value for i, value in enumerate(d)}

    def d_rule(m, i):
        return m.y[i] + sum(m.z[j,i] for j in base_model.I if j != i) <= dd[i]
    base_model.d_constraint = pyo.Constraint(base_model.I, rule=d_rule)

    if 'z_constraint' in base_model.component_map(Constraint):
        del base_model.z_constraint
    if 'z_constraint_index' in base_model.component_map(Set):
        del base_model.z_constraint_index
    ...
    def z_rule(m,i,j):
        ...
        if i == j:
            ...
            return m.z[i,j] <= cfg.detdata["alpha"][str(i)][j-1] * (dd[j]-m.y[j])
    base_model.z_constraint = pyo.Constraint(base_model.I, base_model.I, rule=z_rule)
...
def p_calc(d, cfg):
    ...
    if np.any(np.array(d) == 0):
        print("sampled a zero one!!")

    SF = 1
    if np.isscalar(d[0]):
        pdf_high = SF * norm.pdf(d, detdata["mean_d"]["high"], detdata["stdev_d"]["high"])
        pdf_low = SF * norm.pdf(d, detdata["mean_d"]["low"], detdata["stdev_d"]["low"])
        prob = np.prod((pdf_high + pdf_low)/2)
        return prob / (SF**(len(d)))

    else:
        pdf_high = SF * norm.pdf(d, detdata["mean_d"]["high"], detdata["stdev_d"]["high"])
        # print(pdf_high.shape)
        pdf_low = SF * norm.pdf(d, detdata["mean_d"]["low"], detdata["stdev_d"]["low"])
        # print(pdf_low.shape)
        prob = np.prod((pdf_high + pdf_low)/2, axis=1)
        # print(prob.shape)
        # quit()
        return prob / (SF**(len(d[0])))

def find_limits(cfg):
    detdata = cfg.detdata
    d_limits = []
    aa = 0.0001
    z_score = norm.ppf(1-aa)

    for i in range(cfg.detdata['num_prods']):
        d_limits.append([max(0,detdata["mean_d"]["low"] - z_score * detdata["stdev_d"]["low"]) , detdata["mean_d"]["high"]+ z_score * detdata["stdev_d"]["high"]])
