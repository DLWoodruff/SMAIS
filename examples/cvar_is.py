...
def g_calc(d, base_model, cfg, xhat):
    ...
    def excess_rule(m):
        return m.nu >= d - m.eta
    base_model.excess_constraint = pyo.Constraint(rule=excess_rule)

    nlens = {"ROOT":len(d)}
    return solve_fixed_up_to_stage(base_model, 1, xhat, nlens, cfg.solver_name, cfg.solver_options)
    
def p_calc(d, cfg):
    assert norm.pdf(d)[0] > sys.float_info.epsilon, f'pdf below epsilon {norm.pdf(d)[0]}' 
    if np.isscalar(d[0]):
        return norm.pdf(d)[0]
    else:
        return norm.pdf(d).flatten()

def find_limits(cfg):
    aa = 0.000001
    d_limits = [[norm.ppf(aa), norm.ppf(1-aa)]]
    return d_limits
